...
import { inputTextStyle } from "../inputs/InputTextStyle";
import { SelectDropdown } from "@/app/icons/general";
...
const DateSelect = ({
  ...
  const clickAwayRef = useClickAway<HTMLDivElement>({
    clickAwayCallback: handleClose
  });

  const outlineStyle = () => {
    if (error) return "border-red border-solid";
    if (open) return "border-black border-solid";
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center jusfity-between bg-white py-3 px-4 block w-full rounded-xs-plus border-1 ${outlineStyle()}`}
      >
        <input
          autoComplete="off"
        ...
        <SelectDropdown className={`${open ? "rotate-180" : ""}`} />
      </div>
      {open && (
