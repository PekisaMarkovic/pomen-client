          ...
          <div className="flex-1 flex flex-wrap gap-1">
            <MultySelectedOption
              ...
              handleOnClick={() => {
                const newValues = selected.filter(
                  (opt: SelectOption) => opt.id != selected[0].id
                );
            ...
            {selected.length > 1 && (
              <MultySelectedOption
                ...
                handleOnClick={() => {
                  const newValues = selected.filter(
                    (opt: SelectOption) => opt.id != selected[1].id
                  );
                  setValue(name, newValues);
                }}
              />
      ...
      {isExtended && selected && (
        <div
          className={`flex-1 flex flex-wrap gap-1 ${
            selectedVals.length > 0 ? "my-4" : "mt-4"
          }`}
        >
          {selected
            ...
            .map((sel: SelectOption, index: number) => (
              <MultySelectedOption
                ...
                handleOnClick={() => {
                  setIsSearching(false);
                  const newValues = selected.filter(
                    (opt: SelectOption) => opt.id != sel.id
                  );
                  setValue(name, newValues);
                }}
              />
                    ...
                    <CheckboxOption
                      ...
                      handleOnClick={() => {
                        const newValue = selected.filter(
                          (opt: SelectOption) => opt.id != selectedOpt.id
                        );
